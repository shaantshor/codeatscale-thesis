...
(function () {
 ...
 function __classify(v, depth, seen) {
  ...
  try {
   ...
   if (Array.isArray(v)) {
    // Container-identity cycle guard: a self-referential array ('var a = []; a.push(a)')
    // used to be safe only by accident, since the old code never recursed into non-object
    // elements at all. Once every element started getting fully classified (nesting fix
    // below), a self-referential array produced an entry tree that was itself circular,
    // which __collectHeap then walked with no cycle guard of its own - an uncaught stack
    // overflow that would abort the whole trace, mirroring the same real crash confirmed
    // against real Pyodide WASM on the Python side. Reuses the same mutable 'seen' WeakSet
    // already used for object cycle detection.
    if (seen && seen.has(v)) return { k: 'list', r: '<circular list>', items: ['<circular>'] }
    ...
    // Every element is fully classified (not just direct user objects) so an array/dict
    // nested inside this array can carry its own objects through to __collectHeap too -
    // not just direct object elements. 'items' (above) stays display-strings-only and
    // untouched - that's what the array-box UI reads; item_entries is purely additive,
    // only consumed by heap collection, mirroring the same fix on the Python side
    // (python.worker.js).
    var hasStruct = false
    var itemEntries = sliced.map(function(el) {
     var elEntry = __classify(el, depth + 1, seen)
     if (elEntry.k === 'obj' || elEntry.k === 'ref' ||
       (elEntry.k === 'list' && elEntry.item_entries) ||
       (elEntry.k === 'dict' && elEntry.entries)) {
      hasStruct = true
      return elEntry
     }
     return null
    })
    if (hasStruct) listEntry.item_entries = itemEntries
    return listEntry
   }
   ...
   if (typeof v === 'object') {
    ...
    // Same container-identity cycle guard as the array branch above, for a self-referential
    // plain object ('var d = {}; d.self = d') - dict values were already recursed into
    ...
    // identical.
    if (seen && seen.has(v)) return { k: 'dict', r: '<circular dict>' }
    if (!seen) seen = new WeakSet()
    seen.add(v)
    ...
    try {
     ...
     for (var di = 0; di < dkeys.length; di++) {
      ...
      if (dvEntry.k === 'obj' || dvEntry.k === 'ref' ||
        (dvEntry.k === 'list' && dvEntry.item_entries) ||
        (dvEntry.k === 'dict' && dvEntry.entries)) hasObjVal = true
